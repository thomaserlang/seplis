...
@pytest.mark.asyncio
@respx.mock
async def test_movie(client: AsyncClient) -> None:
    await user_signin(
        client,
        scopes=['movie:create', 'movie:edit', 'movie:delete', 'movie:manage_images'],
    )
    ...
    assert sorted(data.alternative_titles) == sorted(
        ['National Treasure test', 'National Treasure 2004']
    )
