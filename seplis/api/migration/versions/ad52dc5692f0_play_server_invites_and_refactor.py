...
def upgrade() -> None:
    ...
    for id_ in old_ids:
        ...
        conn.execute(
            sa.text(
                'UPDATE play_access SET play_server_id=:new_id WHERE play_server_id=:id'
            ),
            {
                'id': id_,
                'new_id': ids[id_],
    ...
    op.alter_column(
        'play_servers', 'id', existing_type=sa.String(50), type_=UUID, primary_key=True
    )
    ...
    op.alter_column(
        'play_servers', 'created', existing_type=sa.DateTime, new_column_name='created_at'
    )
    op.alter_column(
        'play_servers', 'updated', existing_type=sa.DateTime, new_column_name='updated_at'
    )
