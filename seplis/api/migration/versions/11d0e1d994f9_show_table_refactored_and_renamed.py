...
def upgrade() -> None:
    ...
    op.alter_column(
        'episodes', 'show_id', existing_type=sa.Integer, new_column_name='series_id'
    )
    op.rename_table('episode_watching', 'episode_last_finished')
    op.alter_column(
        'episode_last_finished',
        'show_id',
        existing_type=sa.Integer,
        new_column_name='series_id',
    )
    ...
    op.execute(
        'update series set description_text=null where length(description_text)>2000;'
    )
    op.alter_column(
        'series',
        'description_text',
    ...
    op.execute(
        'update episodes set description_text=null where length(description_text)>2000;'
    )
    op.alter_column(
        'episodes',
        'description_text',
