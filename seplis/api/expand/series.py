...
async def expand_series(
    expand: list[str], user: schemas.User, series: list[schemas.Series]
) -> None:
    if not expand:
        return
    if not user:
...
async def expand_user_watchlist(
    user_id: int, series: list[schemas.Series], session: AsyncSession
) -> None:
    _series: dict[int, schemas.Series] = {}
    for s in series:
        s.user_watchlist = schemas.Series_watchlist()
        _series[s.id] = s
    result: list[models.MSeriesWatchlist] = await session.scalars(
        sa.select(
...
async def expand_user_favorite(
    ...
    for s in series:
        ...
        _series[s.id] = s
    result: list[models.MSeriesFavorite] = await session.scalars(
        sa.select(
...
async def expand_user_can_watch(
    user_id: int, series: list[schemas.Episode], session: AsyncSession
) -> None:
...
async def expand_user_last_episode_watched(
    ...
    result: list[models.MEpisode] = await session.scalars(
    ...
    for episode in result:
        _series[
            episode.series_id
        ].user_last_episode_watched = schemas.Episode.model_validate(episode)
...
async def expand_user_rating(
    ...
    for s in series:
        ...
        _series[s.id] = s
    result: list[models.MSeriesUserRating] = await session.scalars(
        sa.select(
