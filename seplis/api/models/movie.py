...
class Movie(Base):
    ...
    async def _save_genres(session: AsyncSession, movie_id: str | int, genres: list[str | int], patch: bool) -> list[schemas.Genre]:
        ...
        if new_genre_ids != current_genres:            
            await session.execute(sa.text('update genres set number_of = (select count(genres.id) from movie_genres where movie_genres.genre_id = genres.id) where type="movie"'))
