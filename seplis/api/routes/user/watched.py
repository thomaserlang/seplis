...
async def get_user_watched(
    ...
    series_sql = sa.select(
        ...
        models.MEpisodeWatched.episode_number == models.MEpisodeLastWatched.episode_number,
    )
    if user_can_watch:
        series_sql = series_sql.where(
            ...
            models.MPlayServerEpisode.episode_number == models.MEpisodeWatched.episode_number,
        )
    ...
    data = {}
    series = await session.scalars(sa.select(models.MSeries).where(models.MSeries.id.in_(ids['series_ids'])))
    for s in series:
        data[f'series-{s.id}'] = schemas.Series.model_validate(s)
    movies = await session.scalars(sa.select(models.MMovie).where(models.MMovie.id.in_(ids['movie_ids'])))
    for m in movies:
