...
@router.get(
    ...
    response_model=schemas.Episode_watched,
    description="""
...
async def get_watched(
    ...
    ew = await session.scalar(
        sa.select(models.MEpisodeWatched).where(
...
async def watched_increment(
    ...
    data = (
        schemas.Episode_watched_increment.model_validate(request)
        if request
        else schemas.Episode_watched_increment()
    )
...
async def watched_increment_range(
    ...
    if to_episode_number < from_episode_number:
        raise exceptions.API_exception(
