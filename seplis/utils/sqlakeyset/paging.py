...
from .columns import OC, find_order_key, parse_ob_clause
from .results import Page, Paging
from .serial import InvalidPage


def where_condition_for_page(db: AsyncSession, ordering_columns: list[OC], place: tuple[Any]):
    """
...
async def get_page(db: AsyncSession, selectable, per_page: int, place: Optional[tuple[Any]], backwards: bool) -> Page:
    """
    ...
    if place:
        ...
        condition = where_condition_for_page(db, order_cols, place)
