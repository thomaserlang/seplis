...
async def update_popularity(
    create_movies=True, create_above_popularity: float | None = 1.0
) -> None:
    ...
    async with database.session() as session:
        result = await session.stream(
            sa.select(models.MMovie).options(sa.orm.noload('*'))
        )
        ...
        async for data in get_ids('movie_ids'):
            ...
            if len(insert_data) == 10000:
                await session.execute(
                    sa.insert(models.MMoviePopularityHistory)
                    .prefix_with('IGNORE')
                    .values(insert_data),
                )
                insert_data = []
        ...
        await session.execute(
            sa.update(models.MMovie).values({models.MMovie.popularity: 0}),
        )
        await session.execute(
            sa.update(models.MMovie)
            .values({models.MMovie.popularity: models.MMoviePopularityHistory.popularity})
            .where(
