...
async def test_series(client: AsyncClient):
    ...
    r = await client.post('/1/series', json={})
    ...
    r = await client.get(f'/1/series/{series_id}')
    data = r.json()
    ...
    r = await client.get(f'/1/series/externals/imdb/tt123456799')
    data = r.json()
    assert r.status_code == 200, r.content
    assert data['title'], 'QWERTY'
    
    r = await client.patch(f'/1/series/{series_id}', json={
        'title': 'QWERTY2',
        'description': {
    ...
    r = await client.put(f'/1/series/{series_id}', json={
        'title': 'QWERTY2',
        'description': {
    ...
    r = await client.get(f'/1/series/externals/imdb/tt123456797')
    data = r.json()
    assert r.status_code == 200, r.content
    assert data['title'], 'QWERTY2'
    ...
    r = await client.get(f'/1/series/{series_id}/episodes?per_page=1')
    assert r.status_code == 200, r.content
    episodes = r.json()
    assert len(episodes['items']) == 1, episodes
    assert episodes['items'][0]['number'] == 1
    
    r = await client.get(f'/1/series/{series_id}/episodes?per_page=1&page=2')
    assert r.status_code == 200, r.content
    episodes = r.json()
    assert len(episodes['items']) == 1, episodes
    assert episodes['items'][0]['number'] == 2

    r = await client.get(f'/1/series/{series_id}/episodes?season=1')
    assert r.status_code == 200, r.content
    episodes = r.json()
    ...
    r = await client.get(f'/1/series/{series_id}/episodes?season=2')
    assert r.status_code == 200, r.content
    episodes = r.json()
    assert len(episodes['items']) == 1, episodes
    assert episodes['items'][0]['number'] == 3

    r = await client.get(f'/1/series/{series_id}/episodes/3')
    assert r.status_code == 200, r.content

    r = await client.delete(f'/1/series/{series_id}/episodes/3')
    ...
    #r = await client.get(f'/1/search?title=QWERTY2')
    #assert r.status_code == 200, r.content
    #data = r.json()


    config.data.api.storitch = 'http://storitch'
    r = await client.post(f'/1/series/{series_id}/images', 
        files={
    ...
    r = await client.get(f'/1/series/{series_id}/images')
    ...
    r = await client.delete(f'/1/series/{series_id}/images/{poster_image_id}')
    ...
    r = await client.delete(f'/1/series/{series_id}')
    assert r.status_code == 204, r.content

    r = await client.get(f'/1/series/{series_id}/episodes/2')
    assert r.status_code == 404, r.content
