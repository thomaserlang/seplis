import api from '@/api'
import Player, { IPlayNextProps } from '@/components/player/player'
import { IEpisode } from '@/interfaces/episode'
import { IPlaySourceStream } from '@/interfaces/play-server'
import { ISeries, ISeriesUserSettings } from '@/interfaces/series'
import { episodeTitle } from '@/utils'
...
export default function PlayEpisode() {
    ...
    const episode = useQuery(
        ['episode-title-watched', seriesId, episodeNumber],
        async () => {
            const result = await Promise.all([
                api.get<ISeries>(`/2/series/${seriesId}`),
                api.get<IEpisode>(
                    `/2/series/${seriesId}/episodes/${episodeNumber}`,
                    {
                        params: {
                            expand: 'user_watched',
                        },
                ...
                api.get<ISeriesUserSettings>(
                    `/2/series/${seriesId}/user-settings`,
                ),
            ])
            return {
    ...
    const onAudioChange = (source: IPlaySourceStream) => {
        api.put(`/2/series/${seriesId}/user-settings`, {
            audio_lang: source
                ? `${source.language || source.title}:${source.index}`
                : null,
        })
    }
