import api from '@/api'
import { IPageCursorTotalResult } from '@/interfaces/page'
...
type TRenderFilter = (options?: { onClose: () => void }) => JSX.Element
...
export default function List<S = any>({
    ...
    const loading =
        isInitialLoading ||
        isFetchingNextPage ||
        (isFetching && data?.pages?.length == 1)
    return (
        <>
            <Flex wrap="wrap">
                <Heading
