...
export default function Slider<S = undefined>({
    ...
    const [displayItemCount, setDisplayItemCount] = useState(() =>
        rowWidthItems(),
    )
    const { ref, focusKey } = useFocusable({ onFocus })
    const loadPerPage = 12

    const { data, isInitialLoading, fetchNextPage } = useInfiniteQuery(
        ['slider', title, url],
        ...
        if (event?.code == 'ArrowRight') {
            if (items.length - index >= displayItemCount) {
                if (layout.node.getAttribute('data-view-index') > 1)
                    setIndex(index + 1)
                if (index % loadPerPage === 0) fetchNextPage()
            }
    ...
    if (hideIfEmpty && !isInitialLoading && items.length === 0) return null

    return (
        <FocusContext.Provider value={focusKey}>
            <Box ref={ref}>
                {title && (
                    <Heading className="row-header row-margin">{title}</Heading>
                )}
                {!isInitialLoading &&
    ...
    return (
        <FocusContext.Provider value={focusKey}>
            <HStack ref={ref} width="100%">
                ...
                {items.length > 0 &&
                    [...Array(rowWidthItems() - items.length).keys()].map(
...
export function Card<S = any>({
    ...
    return (
        ...
        >
            ...
            {item.bottomText !== undefined && (
                <TextBox>{item.bottomText}</TextBox>
            )}
        </Flex>
    )
}
    ...
    return (
        <>
            {[...Array(rowWidthItems()).keys()].map((key) => (
                <Skeleton
                    key={`skeleton-${key}`}
                    className="poster-container slider-item"
                    rounded="md"
                    width="100%"
                    height="100%"
                />
...
function PeekCard({
    ...
    if (next) {
        ...
        if (items.length <= index + displayCount) return
    } else {
        itemIndex = index - 1
        if (index < 1) return
    }
    ...
    const nextClick = () => {
        setIndex((index) => {
            fetchNextPage()
            let i = index + displayCount
            if (i > items.length - 1) i = items.length - 1 - displayCount
            return i
        })
    }

    return (
        <div className={`slider-peek ${next ? 'next' : 'prev'}`}>
            {items[itemIndex]?.img && (
                <div className="image">
                    <Image src={`${items[itemIndex].img}@SX320.webp`} />
                </div>
            )}
