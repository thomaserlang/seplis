...
export default function Episodes({ series, defaultSeason = 1 }: IProps) {
    ...
    useEffect(() => {
        if (season != defaultSeason) setSeason(defaultSeason)
    }, [series.id])
    return (
        <Stack>
            <SeasonSelect
...
export function RenderEpisodes({
    ...
    const { data, isFetching } = useQuery(
        ...
        async () => {
            const result = await GetAllCursor<IEpisode>(
                `/2/series/${seriesId}/episodes`,
                {
                    params: {
                        expand: isAuthed()
                            ? 'user_watched,user_can_watch'
                            : null,
    ...
    if (isFetching) return <LoadingEpisodes />
    ...
    return (
        <Wrap>
            ...
            <Flex height="0px" grow="1" basis="300px">
                <WrapItem width="100%"></WrapItem>
            </Flex>
            <Flex height="0px" grow="1" basis="300px">
                <WrapItem width="100%"></WrapItem>
            </Flex>
