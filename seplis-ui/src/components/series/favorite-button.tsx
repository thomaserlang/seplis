import api from '@/api'
import { ISeriesFavorite } from '@/interfaces/series'
...
export default function FavoriteButton({ seriesId }: { seriesId: number }) {
    ...
    const toggleFavorite = useMutation(
        async (favorite: boolean) => {
            ...
            queryClient.setQueryData(['series', 'favorite-button', seriesId], {
                favorite: !favorite,
            })
        },
        {
    ...
    const { isInitialLoading, data } = useQuery(
        ['series', 'favorite-button', seriesId],
        async () => {
            const result = await api.get<ISeriesFavorite>(
                `/2/series/${seriesId}/favorite`,
            )
            return result.data
        },
        {
