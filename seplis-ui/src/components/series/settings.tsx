...
import { ISeries, ISeriesRequest } from '@/interfaces/series'
import {
    Alert,
    AlertDialog,
    AlertDialogBody,
...
export function SeriesNew({ onDone }: { onDone?: (seriesId: number) => void }) {
    const onSave = async (data: ISeriesRequest) => {
        ...
        if (onDone) onDone(result.data.id)
        return result.data
    }
...
export function SeriesForm({
    ...
    onSave: (request: ISeriesRequest) => Promise<ISeries>
    onDelete?: () => void
    seriesId?: number
}) {
    const {
...
function Importers({
    ...
    register: UseFormRegister<ISeriesRequest>
}) {
    return (
...
function Externals({
    ...
    return (
        <Flex gap="0.5rem" grow="1" wrap="wrap">
            <FormControl>
                <FormLabel>IMDb</FormLabel>
                <Input
                    {...register('externals.imdb', { required: true })}
                    type="text"
                />
            </FormControl>
...
function EpisodeType({
    ...
    return (
        <Flex gap="0.5rem" grow="1" direction="column">
            <FormControl>
