...
import { ISeriesCastPerson } from '@/interfaces/series-cast'
...
export default function CastFeatured({ seriesId }: { seriesId: number }) {
    const { isInitialLoading, data } = useQuery(
        ['movie', seriesId, 'featured'],
        async () => {
            const result = await api.get<IPageCursorResult<ISeriesCastPerson>>(
                `/2/series/${seriesId}/cast`,
                {
                    params: {
                        per_page: 12,
                        order_le: 99,
    ...
    return (
        <Stack spacing="0.5rem">
            ...
            <SimpleGrid minChildWidth={'300px'} spacing="0.5rem">
                {isInitialLoading ? (
                    [...Array(3)].map((_, i) => <CastCardSkeleton key={i} />)
