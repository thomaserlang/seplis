...
import { ISeries, ISeriesSeason } from '@/interfaces/series'
import { focusedBorder } from '@/styles'
import { isAuthed, langCodeToLang, secondsToHourMin } from '@/utils'
...
export function SeriesLoad({
    ...
    seriesId: number
    onLoaded?: (movie: ISeries) => void
}) {
    const { isInitialLoading, data } = useQuery<ISeries>(
        ['series', seriesId],
        async () => {
    ...
    if (isInitialLoading) return <SeriesSkeleton />
    ...
    if (!data?.title) return <SeriesWaitingForData series={data} />
...
function Title({ series }: { series: ISeries }) {
    return (
        <Box marginTop="-7px" lineHeight="1.3">
...
function BaseInfo({ series }: { series: ISeries }) {
    return (
        <Wrap spacingX="0.75rem" lineHeight="1.3">
            {series.premiered && (
                <WrapItem>
                    <strong title={series.premiered}>
                        {series.premiered.substring(0, 4)}
                    </strong>
                </WrapItem>
            )}
            {series.language && (
                <WrapItem>{langCodeToLang(series.language)}</WrapItem>
            )}
            {series.runtime ? (
                <WrapItem>{secondsToHourMin(series.runtime)}</WrapItem>
            ) : null}
            <SeasonsText seasons={series.seasons} />
            <EpisodesText seasons={series.seasons} />
            {series.rating ? (
                <WrapItem title="IMDb rating">
                    <StarIcon
                        mr="0.2rem"
                        boxSize={3}
                        color="yellow.400"
...
function Genre({ genre }: { genre: IGenre }) {
    ...
    return (
        <Tag ref={ref} style={focused ? focusedBorder : {}}>
            {genre.name}
        </Tag>
    )
}
...
function ExternalLinks({ series }: { series: ISeries }) {
    if (!series.externals.imdb && !series.externals.themoviedb) return
    return (
        <Wrap>
            <WrapItem>
                {series.externals.imdb && (
                    <Link
                        href={`https://imdb.com/title/${series.externals.imdb}`}
                        isExternal
                    >
                        IMDb
                    </Link>
            ...
            <WrapItem>
                {series.externals.themoviedb && (
                    <Link
                        href={`https://www.themoviedb.org/tv/${series.externals.themoviedb}`}
                        isExternal
                    >
                        TMDb
                    </Link>
...
function UserEpisodes({ series }: { series: ISeries }) {
    if (series.seasons.length == 0) return
    if (!isAuthed()) return
    return (
        <Flex gap="0.5rem" align="stretch" wrap="wrap" direction="row">
