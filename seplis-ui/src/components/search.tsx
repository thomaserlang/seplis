import api from '@/api'
import { ITitleSearchResult as ISearchTitleResult } from '@/interfaces/search'
...
export function Search({
    ...
    afterSelected?: (item: ISearchTitleResult) => void
}) {
            ...
            {!isFetching && (
                <RenderItems
                    ...
                    onSelected={(item) => {
                        ...
                        if (afterSelected) afterSelected(item)
                    }}
                />
            )}
        </Stack>
...
function RenderItems({
    ...
    items: ISearchTitleResult[]
    onSelected: (item: ISearchTitleResult) => void
}) {
        ...
        >
            ...
            <Stack direction="column" lineHeight="1.3" spacing="0.25rem">
                ...
                <Stack direction="row" spacing="0.75rem">
                    {item.release_date && (
                        <Text fontWeight="bolder" title={item.release_date}>
                            {item.release_date.substring(0, 4)}
                        </Text>
                    )}
                    {item?.language ? (
                        <Text>{langCodeToLang(item.language)}</Text>
                    ) : null}
                    {item?.runtime ? (
                        <Text>{secondsToHourMin(item.runtime)}</Text>
                    ) : null}
                    {item?.rating ? (
                        <Text title="IMDb rating">{item.rating} IMDb</Text>
                    ) : null}
                </Stack>
...
function ShowType({ type }: { type: string }) {
    if (type === 'movie') return <Tag colorScheme="green">Movie</Tag>
    else if (type === 'series') return <Tag colorScheme="blue">Series</Tag>
    return null
}
