...
export function MovieLoad({
    movieId,
    onLoaded,
}: {
    movieId: number
    onLoaded?: (movie: IMovie) => void
}) {
    const { isInitialLoading, data } = useQuery<IMovie>(
        ['movie', movieId],
        async () => {
    ...
    if (isInitialLoading) return <MovieSkeleton />
    ...
    if (!data?.title) return <SeriesWaitingForData movie={data} />
...
function ListButtons({ movie }: { movie: IMovie }) {
    return (
        <Flex gap="0.5rem" padding="0.25rem 0" wrap="wrap">
...
function BaseInfo({ movie }: { movie: IMovie }) {
    return (
        <Wrap spacingX="0.75rem" lineHeight="1.3">
            {movie.release_date ? (
                <WrapItem>
                    <strong title={movie.release_date}>
                        {movie.release_date.substring(0, 4)}
                    </strong>
                </WrapItem>
            ) : null}
            {movie.language ? (
                <WrapItem>{langCodeToLang(movie.language)}</WrapItem>
            ) : null}
            {movie.runtime ? (
                <WrapItem>{secondsToHourMin(movie.runtime)}</WrapItem>
            ) : null}
            {movie.rating ? (
...
function ExternalLinks({ movie }: { movie: IMovie }) {
    if (!movie.externals.imdb && !movie.externals.themoviedb) return
    return (
        <Wrap>
            ...
            {movie.externals.themoviedb && (
                <WrapItem>
                    <Link
                        href={`https://www.themoviedb.org/movie/${movie.externals.themoviedb}`}
                        isExternal
                    >
                        TMDb
                    </Link>
