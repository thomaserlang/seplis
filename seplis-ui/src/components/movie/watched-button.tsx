...
export function MovieWatchedButtonData({
    movieId,
    data,
}: {
    movieId: number
    ...
    useEventListener<IEventMovieWatched>(
        EVENT_MOVIE_WATCHED,
        (data) => {
            if (data.movieId == movieId) setWatched({ ...data.watched })
        },
        [movieId],
    )
    ...
    const increment = async () => {
        try {
            setUpdating(true)
            const r = await api.post<IMovieWatched>(
                `/2/movies/${movieId}/watched`,
            )
    ...
    const decrement = async () => {
        try {
            setUpdating(true)
            const r = await api.delete<IMovieWatched>(
                `/2/movies/${movieId}/watched`,
            )
            TriggerMovieWatched(movieId, r.data)
        } catch (e) {
...
export function MovieWatchedButton({ movieId }: { movieId: number }) {
    const { isInitialLoading, data } = useQuery(
        ...
        async () => {
            const result = await api.get<IMovieWatched>(
                `/2/movies/${movieId}/watched`,
            )
            return result.data
        },
