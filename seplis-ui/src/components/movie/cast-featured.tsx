import api from '@/api'
import { IMovieCastPerson } from '@/interfaces/movie-cast'
...
export default function CastFeatured({ movieId }: { movieId: number }) {
    const { isInitialLoading, data } = useQuery(
        ['movie', movieId, 'featured'],
        async () => {
            const result = await api.get<IPageCursorResult<IMovieCastPerson>>(
                `/2/movies/${movieId}/cast`,
                {
                    params: {
                        per_page: 12,
                        order_le: 100,
